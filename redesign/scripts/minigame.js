...
(() => {
  ...
  const DRONE_SPAWN_BASE_OFFSET = 70;
  ...
  Object.entries(SPRITE_FILES).forEach(([key, path]) => {
    ...
    im.src = (useSweethope ? SWEETHOPE_ASSET_BASE : ASSET_BASE) + path
  ...
  function getPlayerDieDuration() {
    return Math.max(0.85, clipDuration(getPlayerDieKey(), ANIM_FPS.die));
  }
